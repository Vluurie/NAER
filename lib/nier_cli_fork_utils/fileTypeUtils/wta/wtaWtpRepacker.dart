...
Future<void> repackWtaWtp(String wtaPath, String? wtpPath, String extractedDir,
    {bool isWtb = false}) async {
  ...
  for (var ddsFile in ddsFiles) {
    ...
    if (parts == null) {
      throw Exception(
          "Invalid dds file name: $ddsFile! Expected format: <index>_<hex_id>.dds");
    }
  ...
  List<int> textureFlags =
      List.generate(ddsFilesData.length, (_) => 0x20000020);
  List<int> textureIds = [];
  int currentOffset = 0;
  for (var ddsFile in ddsFilesData) {
    ...
    var padding =
        List.filled(remainingPadding(currentOffset, isWtb ? 4096 : 16), 0);
    currentOffset += padding.length;
  }

  var wtaHeader = WtaFileHeader.empty();
  var wtaFile = WtaFile(
      wtaHeader, textureOffsets, textureSizes, textureFlags, textureIds, []);
  ...
  try {
    wtp = await File(isWtb ? wtaPath : wtpPath!)
        .open(mode: isWtb ? FileMode.append : FileMode.write);
    for (int i = 0; i < textureOffsets.length; i++) {
