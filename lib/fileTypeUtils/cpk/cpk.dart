...
class CpkTableDataPool {
  ...
  int get currentRowStorePos =>
      tableStartOffset +
      8 +
      rowsStoreOffset +
      row * rowsStoreRowLength +
      offset;
}
...
class CpkField {
  ...
  CpkField.read(ByteDataWrapper bytes, CpkTableHeader header) {
    ...
    if (isDefaultValue && isRowStorage)
      throw Exception(
          "Field cannot have both DefaultValue and RowStorage flags");
...
class CpkFileCompressed extends CpkFile {
  ...
  Uint8List decompress() {
    ...
    while (bytesOutput < uncompressedSize) {
      if (getNextBits(compressedData, 1, bitsInfo) > 0) {
        int backReferenceOffset = outputEnd -
            bytesOutput +
            getNextBits(compressedData, 13, bitsInfo) +
            3;
        ...
        for (vleLevel = 0; vleLevel < vleLens.length; vleLevel++) {
          int this_level =
              getNextBits(compressedData, vleLens[vleLevel], bitsInfo);
        ...
        if (vleLevel == vleLens.length) {
          int this_level;
          do {
            this_level = getNextBits(compressedData, 8, bitsInfo);
            backReferenceLength += this_level;
        ...
        result[outputEnd - bytesOutput] =
            getNextBits(compressedData, 8, bitsInfo);
        bytesOutput++;
      }
    }
  ...
  int getNextBits(Uint8List input, int bitCount, CriLaylaBitInfo bitsInfo) {
    ...
    while (numBitsProduced < bitCount) {
      ...
      outBits |= (bitsInfo.bitPool >> (bitsInfo.bitsLeft - bitsThisRound)) &
          ((1 << bitsThisRound) - 1);
...
class Cpk {
  ...
  Cpk.read(ByteDataWrapper bytes) : header = CpkSection.read(bytes) {
    ...
    for (var i = 0; i < toc!.table.header.rowCount; i++) {
      ...
      // remove folder without enemys
      var validFolderNames = [
        "ph4",
        "st5",
        "quest",
        "ph2",
