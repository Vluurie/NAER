...
import '../fileTypeUtils_fork/utils/ByteDataWrapper.dart';
...
// void Function() throttle(void Function() func, int waitMs,
...
// void Function() debounce(void Function() func, int waitMs,
//     {bool leading = false}) {
//   Timer? timeout;
//   return () {
//     timeout?.cancel();
...
//     XmlName(name),
//     attributes.entries
//         .map((attr) => XmlAttribute(XmlName(attr.key), attr.value))
//         .toList(),
//     <XmlNode>[
...
//   if (!await File(pakInfoPath).exists()) return [];
...
//   if (!await File(pakInfoPath).exists()) return null;
//   Map pakInfoJson = jsonDecode(await File(pakInfoPath).readAsString());
...
//   if (fileInfoIndex == -1) return null;
//   return pakInfoJson["files"][fileInfoIndex];
// }

// Future<void> updatePakInfoFileData(
//     String path, void Function(dynamic data) updater) async {
//   var pakInfoPath = join(dirname(path), "pakInfo.json");
//   if (!await File(pakInfoPath).exists()) return;
//   Map pakInfoJson = jsonDecode(await File(pakInfoPath).readAsString());
//   var yaxName = "${basenameWithoutExtension(path)}.yax";
...
String getDatFolder(String datName) {
  ...
  var c1 = datName[0];
  if (c1 == "r") return "st${datName[1]}";
  if (c1 == "p") return "ph${datName[1]}";
  if (c1 == "g") return "wd${datName[1]}";

  for (var start in _nameStartToFolder.keys) {
    if (datName.startsWith(start)) return _nameStartToFolder[start]!;
  }

  if (isInt(c2)) return path.join("effect", "model");
...
//   if (number == 1) return "$number$numberSuffix $label";
...
// List<T> spaceListWith<T>(List<T> list, T Function() generator,
//     [bool outer = false]) {
//   var newList = <T>[];
//   for (var i = 0; i < list.length; i++) {
//     if (i != 0 || outer) newList.add(generator());
//     newList.add(list[i]);
//   }
//   if (outer) newList.add(generator());
//   return newList;
// }
...
//   if (showMs) {
//     var ms =
//         ((duration.inMilliseconds) % 1000).toInt().toString().padLeft(3, "0");
//     return "$mins:$secs.$ms";
//   }
//   return "$mins:$secs";
// }
...
//     if (basename(path) == _assetsDirName &&
//         _assetsDirSubDirs.every((subDir) => subDirNames.contains(subDir))) {
//       _assetsDir = path;
//       return path;
//     }
...
//   if (basename(exePath) == "dart.exe") return currentDir;
